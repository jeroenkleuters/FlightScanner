...
- [ ] **Strip the Vite demo content.** Remove the counter, logos, demo CSS, and
...
- [ ] **Add ESLint and Prettier.** TypeScript and React Hooks rules, Prettier for
...
- [ ] **Add `src/config.ts` and `.env.example`.** Variables, all `VITE_`
...
- [ ] **Add the `verify` script.** `tsc --noEmit && vitest run && vite build`, in
...
- [ ] **Update the Commands section of `AGENTS.md`.** Replace the Next.js
